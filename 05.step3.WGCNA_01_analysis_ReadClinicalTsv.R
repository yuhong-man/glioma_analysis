...
### 追踪临床数据
#############################################################################################
# traitData <- read.csv("clinical.dedup._.tsv",sep="\t",row.names=1, fill=T)
traitData <- read.csv("clinical_data_with_condition.tsv",sep="\t",row.names=1, fill=T)
...
print('allTraits$case_submitter_id')
print(allTraits$case_submitter_id)

intersected_tcga_ids <- intersect(rownames(datExpr),allTraits$case_submitter_id)
print('intersected_tcga_ids')
print(intersected_tcga_ids)
print(dim(datExpr))
datExpr <- datExpr[intersected_tcga_ids,] 
print(dim(datExpr))


traitRows <- match(rownames(datExpr),allTraits$case_submitter_id);
print('traitRows')
print(traitRows)
datTraits <- allTraits[traitRows, -1];  # Remove the last column. 
