...
# args[3] PC1 大于'g' 还是小于'l'
# args[4] PC1 值筛选
# args[5] PC2 大于'g' 还是小于'l'
# args[6] PC2 值筛选
...
filter_data <- function(pcaData,PC1_d,PC1_v,PC2_d,PC2_v){
	if(PC1_d == 'g'&PC2_d =='l'){
		outlier_name <- pcaData[pcaData$PC1 > PC1_v&pcaData$PC2 < PC2_v,]
	}
...
outlier_name <- filter_data(pcaData,args[3],as.numeric(args[4]),args[5],as.numeric(args[6]))
